//
// Created by anonymous on 05.08.21.
//
    ...
    void computeEffectsOfMethodVariable(Model* model, int var, vector<int>* poss_eff_positive, vector<int>* poss_eff_negative,
        ...
        for (int index1 = 0; index1 < model->numMethods; index1++) {
            ...
            for (int index2 = method_indexes[index1]; index2 < model->numSubTasks[index1]; index2++) {
                ...
                if (compoundTaskIndex < 0) {
                    if (available_actions[nextTaskInTN] == 1){
                        if (poss_eff_positive[decomposedTaskIndex].empty() 
                                || poss_eff_positive[decomposedTaskIndex].back() != var){
                            poss_eff_positive[decomposedTaskIndex].push_back(var);
                        } 
                    } else if (available_actions[nextTaskInTN] == -1){
                        if (poss_eff_negative[decomposedTaskIndex].empty()
                                || poss_eff_negative[decomposedTaskIndex].back() != var){
        ...
        while (state_added) {
            ...
            for (int index1 = 0; index1 < amount_compound_tasks; index1++) {
                ...
                for(auto const& index2: adjacent[index1]){
                        ...
                        if (!poss_eff_positive[index1].empty() && poss_eff_positive[index1].back() == var 
                        && (poss_eff_positive[index2].empty() || !(poss_eff_positive[index2].back() == var))) {
                            poss_eff_positive[index2].push_back(var);
                            state_added = true;
                        }
                        if (!poss_eff_negative[index1].empty() && poss_eff_negative[index1].back() == var 
                        && (poss_eff_negative[index2].empty() || !(poss_eff_negative[index2].back() == var))) {
    ...
    void computeRelaxedMethodPreconditions(Model* model, int var, vector<int>* preconditions,
        ...
        do {
            ...
            for (int index1 = 0; index1 < model->numMethods; index1++) {
                ...
                for (int index2 = 0; index2 <= method_indexes[index1]; index2++) {
                    ...
                    if (c_task_index < 0) {                    
                        if (!available_actions[nextTaskInTN][0] && available_actions[nextTaskInTN][1]) {
                        ...
                        else if (available_actions[nextTaskInTN][0] && !available_actions[nextTaskInTN][1]) {
        ...
        for (int index1 = 0; index1 < model->numMethods; index1++) {
            for (int index2 = 0; index2 <= method_indexes[index1]; index2++) {
                ...
                } else if (prec_of_c_tasks[compoundTaskIndex]) {
                    model->prec_m[index1].push_back(var);
                    break;
                } else if (c_produces_f_first[compoundTaskIndex]) {
                    break;
